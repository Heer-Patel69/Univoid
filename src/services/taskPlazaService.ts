...
// Explicit column selection for tasks
const TASK_REQUEST_COLUMNS = 'id, requester_id, task_type, title, description, subject, page_count, deadline, budget, is_negotiable, attachment_urls, status, assigned_to, created_at, updated_at';
const TASK_BID_COLUMNS = 'id, task_id, solver_id, message, status, created_at';

// Get all open tasks
export async function getOpenTasks(): Promise<TaskRequest[]> {
  const { data, error } = await supabase
    .from('task_requests')
    .select(TASK_REQUEST_COLUMNS)
    .eq('status', 'open')
