...
export const FloatingDoodles = ({ 
  ...
  const densityCount = {
    ...
    medium: 20,
    high: 32,
    global: 45, // Increased density for global background
  };
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (section === "hero") {
        // Place more doodles on edges for hero
  ...
  return (
    ...
    >
      {doodles.map((doodle) => {
        // Calculate parallax offset (opposite direction, very subtle)
        const parallaxX = -mousePosition.x * 15 * (doodle.size / 20);
        const parallaxY = -mousePosition.y * 15 * (doodle.size / 20);
